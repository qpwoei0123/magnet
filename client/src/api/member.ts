...
export const getMember = async (): Promise<GetMemberResponse> => {
	...
	const roles = ['USER'];
	if (mentorProfile) roles.push('MENTOR');
	if (menteeEnrollments.length > 0) roles.push('MENTEE');
	...
	MemberStore.getState().setGlobalMember(memberData);
	return memberData;
};
...
export const updateMember = async (params: UpdateMemberParams) => {
	...
	MemberStore.getState().setGlobalMember(updatedMemberData);
};
